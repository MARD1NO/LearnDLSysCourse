...
def xavier_uniform(fan_in, fan_out, gain=1.0, **kwargs):
    ### BEGIN YOUR SOLUTION
    a = gain * numpy.sqrt(6 / (fan_in + fan_out))
    return ndl.Tensor(numpy.random.uniform(-a, a, size=(fan_in, fan_out)), **kwargs)
    ### END YOUR SOLUTION
...
def xavier_normal(fan_in, fan_out, gain=1.0, **kwargs):
    ### BEGIN YOUR SOLUTION
    mean = 0 
    std = gain * numpy.sqrt(2 / (fan_in + fan_out))
    return ndl.Tensor(numpy.random.normal(mean, std, size=(fan_in, fan_out)), **kwargs)
    ### END YOUR SOLUTION
...
def kaiming_uniform(fan_in, fan_out, nonlinearity="relu", **kwargs):
    ...
    ### BEGIN YOUR SOLUTION
    bound = numpy.sqrt(2) * numpy.sqrt(3 / (fan_in))
    return ndl.Tensor(numpy.random.uniform(-bound, bound, size=(fan_in, fan_out)), **kwargs)
    ### END YOUR SOLUTION
...
def kaiming_normal(fan_in, fan_out, nonlinearity="relu", **kwargs):
    ...
    ### BEGIN YOUR SOLUTION
    mean = 0 
    std = numpy.sqrt(2) / numpy.sqrt(fan_in)
    return ndl.Tensor(numpy.random.normal(mean, std, size=(fan_in, fan_out)), **kwargs)
    ### END YOUR SOLUTION
