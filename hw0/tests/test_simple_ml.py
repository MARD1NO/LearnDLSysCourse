...
def test_softmax_regression_epoch():
    ...
    # mnist_X, mnist_y = parse_mnist("data/train-images-idx3-ubyte.gz",
    #                   "data/train-labels-idx1-ubyte.gz")
    # X = mnist_X[:50, :]
    # y = mnist_y[:50]
    # Theta = np.zeros((784, 10), dtype=np.float32)
    # dTheta = -nd.Gradient(lambda Th : softmax_loss(X@Th.reshape(784, 10),y))(Theta)
    # softmax_regression_epoch(X,y,Theta,lr=1.0,batch=50)
    # np.testing.assert_allclose(dTheta.reshape(784, 10), Theta, rtol=1e-4, atol=1e-4)
